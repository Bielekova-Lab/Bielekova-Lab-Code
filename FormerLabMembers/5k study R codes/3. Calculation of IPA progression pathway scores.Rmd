---
title: "Calculation of IPA progression pathway scores in 5k dataset"
output:
  html_document:
    df_print: paged
...
THIS SCRIPT calculates progression pathway scores for global MS disability outcome

_ only pathways with -log10(pval)>5 were considered

- GMS disability coefficient used ot assign directionality 

- scores calcluated using HD age/sex-adjusted and HD scaled somamers (3714 somamers - human only) with outliers CURBED
...
pathway_data <- pathway_data %>% 
  filter(outcome=="Disability progression: GMS disability") %>% 
  filter(pval>5)
...
# load correlation of somamers with GMSdis outcome
  ...
  corr <- read_csv("./input/gmsdis_forIPA_selection.csv")
  
  corr$assigned_directionality <- ifelse(corr$gmsdis_coef<0,yes = -1,no = 1)
...
############################
#########################
PROGRESSION PATHWAY SCORES calculation
#########################
############################
...
markers <- strsplit(selection$molecules,"[,/ -()]")[[1]]
  ...
  final_data[,samples] <- final_data$gmsdis_coef * final_data[,samples]
  
  cor_dir_sum <- c(cor_dir_sum,sum(final_data$gmsdis_coef))
  # cor_dir_mean <- c(cor_dir_mean,mean(final_data$gmsdis_coef))
  ...
  cor_dir <- ifelse(sum(final_data$gmsdis_coef)>0,yes = 1,no = -1)
  ...
  for_summary <- final_data %>% 
    ...
    mutate(method="IPA") %>% 
    dplyr::select(pathway,type,method,somaid,gene,gmsdis_coef) %>% 
    dplyr::rename(coefficient=gmsdis_coef)
...
write_csv(nr_soma_for_scores,"./output/pathway_scores/curbed/nr_soma_in_scores_08072024.csv")


coef_data <- tibble::enframe(pathways,name = NULL,value = "pathway")

# coef_data$GMSdis_Rho_mean <- cor_dir_mean
coef_data$GMSdis_Rho_sum <- cor_dir_sum
...
write_csv(final_scores_tosave,"./output/pathway_scores/progression_pathways_scores.csv")
...
write_csv(pathway_soma,"./output/pathway_scores/soma_selection_in_progression_pathways.csv")

# save summary pathways 

write_csv(summary_pathways,"./output/pathway_scores/progression_summary_pathways.csv")
```
