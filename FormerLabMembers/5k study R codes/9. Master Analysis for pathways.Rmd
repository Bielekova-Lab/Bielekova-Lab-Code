---
title: "Masterspreadsheet of pathway scores in 5k dataset"
output:
  html_document:
    df_print: paged
...
THIS SCRIPT generates a masterspreadsheet of:
1. IPA progression pathway scores for global MS disability outcome
2. New progression pathway scores for global MS disability outcome
3. IPA MS vs HD pathway scores
4. New MS vs HD pathway scores

These pathway scores were calculated using cubrbed dataset
...
output directory "./output/masterspreadsheet
...
demo_vars <- names(demo)[-1]


###########################################################
# load IPA pathways
data_ipa <- read_csv("./input/pathways_all.csv")

# generate a vector of names for IPA pathways 
#progression
prog_ipa_pathways <- readLines("./input/IPA_progression_pathways.txt")
#MS vs HD
ms_ipa_pathways <- readLines("./input/IPA_ms_vs_hd_pathways.txt")


###########################################################
# load new progression pathways
data_new <- read_csv("./input/new_progression_pathways_scores.csv")

# add "prog" at the end of the athway names to identify them 
names(data_new)[-1] <- paste0(names(data_new)[-1],"_prog")

# generate a vector of names for NEW progression pathways 
prog_new_pathways <- names(data_new)[-1]


###########################################################
# load new MS vs HD pathways
data_new_MSvsHD <- read_csv("./inout/new_ms_vs_hd_pathways_scores.csv")

#add "MSvsHD" to pathway names to differentiate them from new progression pathways
names(data_new_MSvsHD)[-1] <- paste0(names(data_new_MSvsHD)[-1],"_MSvsHD")

# generate a vector of names for NEW MS vs HD pathways 
MSvsHD_new_pathways <- names(data_new_MSvsHD)[-1]
...
data <- left_join(data_ipa,data_new)
data <- left_join(data,data_new_MSvsHD)


# generate vector of pathway names

pathways <- c(prog_ipa_pathways,ms_ipa_pathways,prog_new_pathways,MSvsHD_new_pathways)

j <- c(1:length(pathways))

pathways_raw <- paste0(paste0("pathway_",j,"_score"))

# replace names of ipa pathways with coded names

data <- data %>% 
  dplyr::select(sampleid,all_of(pathways))


names(data)[-1] <- pathways_raw

# merge with demo data

data <- right_join(demo,data)

table(data$diagnosis)

# select only untreated MS and HD samples
...
#bd_t2
bd_t2 <- read_csv("./output/differences/bd_vs_t2_propensity_data.csv")
bd_t2 <- bd_t2 %>% 
...
#t2_bd
t2_bd <- read_csv("./output/differences/t2_vs_bd_propensity_data.csv")
t2_bd <- t2_bd %>% 
  dplyr::select(sampleid,mark,subclass) %>% 
  rename(t2_bd=mark)%>% 
...
#bd_combi
bd_combi <- read_csv("./output/differences/bd_vs_combi_propensity_data.csv")
bd_combi <- bd_combi %>% 
  dplyr::select(sampleid,mark,subclass) %>% 
...
#combi_bd
combi_bd <- read_csv("./output/differences/combi_vs_bd_propensity_data.csv")
combi_bd <- combi_bd %>% 
  dplyr::select(sampleid,mark,subclass) %>% 
...
to_plot_age_residuals <- to_plot %>% 
  filter(cohort=="MS") %>% 
  dplyr::select(-all_of(pathways_raw))

to_plot_msonly <- to_plot %>% 
  filter(cohort=="MS")

for(i in 1:length(pathways_raw)){
# i <-226
    formula <- paste0("`",pathways_raw[i],"`~","age")
    
    mod <- lm(formula = formula,data = to_plot_msonly)
    
    to_plot_age_residuals[[pathways_raw[i]]]  <- mod$residuals
    print(i)
  }
...
CORRELATIONS OF PATHWAY SCORES WITH AGE AND PROGRESSION OUTCOMES

####################################################################

```{r}

results <- tibble::enframe(pathways,name = NULL,value = "pathway")

results$marker <- pathways_raw

# generate column that differentiates the origin of pathways

results$origin <- NA

results$origin[which(results$pathway %in% prog_ipa_pathways)] <- "progressive_IPA"
results$origin[which(results$pathway %in% prog_new_pathways)] <- "progressive_NEW"
results$origin[which(results$pathway %in% ms_ipa_pathways)] <- "MSvsHD_IPA"
results$origin[which(results$pathway %in% MSvsHD_new_pathways)] <- "MSvsHD_NEW"

table(results$origin)
...
#HD
suppressWarnings(
corr_hd <- corr_fun(dat = data_hd,outcomes = "age",high_markers = pathways_raw)
)
...
wilc_p <- function(col_num){
      # col_num <- 1
      mod <- summary(lm(dat[[pathways_raw[col_num]]]~dat[[outcome]]))
      
      p <- tibble::enframe(mod$r.squared,name = NULL, value = pathways_raw[col_num])
      out <- (p)
...
#MS
suppressWarnings(
corr_ms <- corr_fun(dat = data_ms,outcomes = "age",high_markers = pathways_raw)
)
...
wilc_p <- function(col_num){
      ...
      p <- tibble::enframe(c(mod$r.squared,
                           ...
                           name = NULL, value = pathways_raw[col_num])
...
#MS - Females
suppressWarnings(
corr_ms <- corr_fun(dat = data_ms_f,outcomes = "age",high_markers = pathways_raw)
)
...
#MS - Males
suppressWarnings(
corr_ms <- corr_fun(dat = data_ms_m,outcomes = "age",high_markers = pathways_raw)
)
...
# correlate with pathways
suppressWarnings(
corr_nfl <- corr_fun(dat = data_ms,outcomes = "NFL_CEL_residuals",high_markers = pathways_raw)
)
...
wilc_p <- function(col_num){
  # col_num <- 1
mod <- summary(lm(dat[[pathways_raw[col_num]]]~dat[[outcome]]*dat$gender))
      f <- mod$fstatistic
...
for(i in 1:length(outcomes)){
  ...
  #MS
suppressWarnings(
corr_ms <- corr_fun(dat = data_ms,outcomes = outcomes[i],high_markers = pathways_raw)
)
...
#MS - Females
suppressWarnings(
corr_ms <- corr_fun(dat = data_ms_f,outcomes = outcomes[i],high_markers = pathways_raw)
)
...
#MS - Males
suppressWarnings(
corr_ms <- corr_fun(dat = data_ms_m,outcomes = outcomes[i],high_markers = pathways_raw)
)
...
#########################################
# statistics for disability outcomes
...
differences <- tibble::enframe(pathways_raw,name = NULL,value = "marker")
differences$MS_HD_median_diff <- NA

for(l in 1:length(pathways_raw)){
      # l <- 1
      differences$MS_HD_median_diff[l] <- median(data_ms[[pathways_raw[l]]]) - median(data_hd[[pathways_raw[l]]])
...
wilc_p <- function(col_num){
      # col_num <- 1
      wilc <- wilcox.test(dat[[pathways_raw[col_num]]]~dat[[var]], alternative = "two.sided")
      
      p <- tibble::enframe(wilc$p.value,name = NULL, value = pathways_raw[col_num])
      out <- (p)
...
for(l in 1:length(pathways_raw)){
      # l <- 1
      differences$MS_F_M_median_diff[l] <- median(data_ms_f[[pathways_raw[l]]]) - median(data_ms_m[[pathways_raw[l]]])
...
# patient_coeffs <- get_patient_specific_coefficients(long_data, subject_col = "patientcode", age_col = "age", variables = pathways_raw)
# 
# # View the results
# head(patient_coeffs)
# 
...
# 
# write_csv(patient_coeffs_clean_wide,"./output/longitudinal_analyses/patient_specific_slopes_pathways.csv")

patient_coeffs_clean_wide <- read_csv("./input/patient_specific_slopes_pathways.csv")
...
wilc_p <- function(col_num){
  # col_num <- 1
  wilc <- wilcox.test(dat[[pathways_raw[col_num]]],mu=0,alternative = "two.sided")
...
for(l in 1:length(pathways_raw)){
      # l <- 1
      difference[[pathways_raw[l]]] <- data_q3[[pathways_raw[l]]] - data_q1[[pathways_raw[l]]]
...
differences <- tibble::enframe(pathways_raw,name = NULL,value = "marker")
differences[[paste0(var[i],"_median_diff")]] <- apply(X = difference[pathways_raw],MARGIN = 2,FUN = median)
  

dat <- rbind(data_q1,data_q3)
wilc_p <- function(col_num){
      # col_num <- 1
      wilc <- wilcox.test(dat[[pathways_raw[col_num]]]~dat[[var[i]]], alternative = "two.sided", paired=TRUE)
...
openxlsx::write.xlsx(results,"./output/masterspreadsheet/progression_pathways_5k_masterspreadsheet.xlsx",asTable = TRUE)

# save as csv

write_csv(results,"./output/masterspreadsheet/pathways_5k_masterspreadsheet.csv")
