---
title: "Generation of new non-IPA pathway in 5k dataset"
output:
  html_document:
    df_print: paged
...
THIS SCRIPT identifies somamers not used in any of the IPA progression pathways and uses 
...
pathway_data <- read_csv("./input/ipa_progression_pathways_soma_summary.csv")
...
# load GMSdis coefficients

  corrs <- read_csv("./input/gmsdis_forIPA_selection.csv")
...
write_csv(newcluster_results,"./output/new_pathways/new_progression_pathways_summary.csv")
...
selection <- pathway_data %>% 
  ...
  pathways_data <- selection %>% 
    dplyr::select(marker,gene,gmsdis_coef)
  ...
  for_summary <- final_data %>% 
    ...
    dplyr::select(pathway,type,method,marker,gene,gmsdis_coef) %>% 
...
write_csv(nr_soma_for_scores,"./output/new_pathways/nr_soma_in_scores.csv")




write_csv(results,"./output/new_pathways/new_progression_pathways_scores.csv")

# save summary pathways 

write_csv(summary_pathways,"./output/new_pathways/new_progression_summary_pathways.csv")
```
